...
export function validateWorkspace(value: unknown): WorkspaceValidation {
  ...
  if (!optional(value.questionFollowUps, items => list(items, item => object(item) && id(item.id) && id(item.questionId) && string(item.prompt, 1000) && item.prompt.trim().length > 0 && oneOf(item.status, ['Waiting for reply', 'Context received']) && date(item.createdAt) && date(item.updatedAt), 500))) return fail('The clarification follow-ups are invalid or exceed the workspace limits.');
  const followUps = workspace.questionFollowUps || [];
  if (new Set(followUps.map(item => item.id)).size !== followUps.length || new Set(followUps.map(item => item.questionId)).size !== followUps.length || followUps.some(item => !questionIds.has(item.questionId))) return fail('A clarification follow-up is repeated or refers to a missing question.');
  if (!optional(value.decisionSections, items => list(items, item => object(item) && id(item.id) && string(item.title, 120) && item.title.trim().length > 0 && ids(item.cardIds) && (item.cardIds as string[]).length > 0 && date(item.createdAt) && date(item.updatedAt), 100))) return fail('The decision sections are invalid or exceed the workspace limits.');
  const sections = workspace.decisionSections || [];
  if (new Set(sections.map(item => item.id)).size !== sections.length || sections.some(item => item.cardIds.some(key => !workspace.cards.some(card => card.id === key)))) return fail('A decision section is repeated or refers to a missing evidence card.');
