import {affectedAnswers} from './issueImpact';
...
export function toPublishedAdvice(draft: Draft, workspace: Workspace): PublishedAdvice {
  ...
  const publicSources=draft.sourceRefs.filter(s=>s.page>0 && !(questionSource && s.page===questionSource.page && s.label===questionSource.label && s.excerpt===questionSource.excerpt) && !/^E-01\./.test(s.label)).map(({page,label,excerpt})=>({page,label,excerpt}));
