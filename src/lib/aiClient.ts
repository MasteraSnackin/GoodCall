...
async function call<T>(path: string, body?: unknown, signal?: AbortSignal): Promise<T> {
  if (isStaticDemo()) throw new Error('Live AI is unavailable in this hosted demo. Use the local evidence templates for answers and chat.');
...
export const getAiStatus = (signal?: AbortSignal) => isStaticDemo() ? Promise.resolve({ ...EMPTY_AI_STATUS }) : call<AiStatus>('status', undefined, signal);
