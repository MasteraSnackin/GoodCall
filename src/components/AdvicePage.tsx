...
import {isAdvice,shareUrl} from '../lib/share';
...
const SAVED='maya-saved-advice-v1';
...
export default function AdvicePage({advice,onBack,preview=false}:{advice:PublishedAdvice|null;onBack:()=>void;preview?:boolean}){
 ...
 const [saved,setSaved]=useState<PublishedAdvice[]>(()=>preview?[]:getSaved());
 const [copied,setCopied]=useState(false);const [fallback,setFallback]=useState('');const [error,setError]=useState('');
 if(!current)return <div className={`advice-page${preview?' advice-preview':''}`}><nav>{!preview&&<button onClick={onBack}><ArrowLeft size={17}/>Back to workspace</button>}<span>GoodCall</span></nav><main><span className="eyebrow">{preview?'PREVIEW · NOT PUBLISHED':'THAT LINK NEEDS ANOTHER LOOK'}</span><h1>We couldn’t open this answer.</h1><p>{preview?'A complete answer is needed before it can be previewed.':'The shared link may be incomplete or invalid. Ask for a fresh link to the approved card.'}</p>{!preview&&<button className="primary" onClick={onBack}>Open workspace</button>}</main></div>;
 const isSaved=saved.some(s=>s.id===current.id&&s.publishedAt===current.publishedAt&&s.text===current.text&&JSON.stringify(s.decision)===JSON.stringify(current.decision));
